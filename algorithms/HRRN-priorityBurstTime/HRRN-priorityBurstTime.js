...
function find(burstTime, n, visited, curTime, priorities, arrivalTime) {
    var mx = -1;
    var ans = -1;
    for (var i = 0; i <= n; i += 1) {
        if (!visited[i] && arrivalTime[i] <= curTime) {
            var metric = curTime - arrivalTime[i];
            var priority =
                (metric + priorities[i] + burstTime[i]) /
                (priorities[i] + burstTime[i]);
            if (priority > mx) {
                mx = priority;
                ans = i;
            }
...
function priorityBurstTime(processes) {
    var burstTime = [];
    var arrivalTime = [];
    var priorities = [];
    var finalSchedule = [];
...
function compute() {
    ...
    var processes = arrivalTimeArr.map((entry, idx) => {
        return {
            ...
            priority: priorityArr[idx],
        };
    });
    //-------------------------------------Main Algorithm (input is array of 'processes')-----------------------------------------

    var slots = priorityBurstTime(processes);
...
function addRow() {
    ...
    cell2.innerHTML =
        '<input type="text" class="priority" id =P' +
        numProcess +
        ' ' +
        'value=' +
