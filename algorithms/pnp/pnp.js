                    ...
                    if (i == GLOBAL_endTime) {
                        ...
                        $('#gantt_chart').append(
                            '<div data-process="' +
                                ...
                                '" data-end="' +
                                GLOBAL_endTime +
                                '" class="gantt_block" style="background-color: ' +
                                my_colors[(cpu_process - 1) % 4] +
                                '; width: ' +
                                curr_width +
                                '%;">P' +
                                cpu_process +
                        ...
                        var curr_arrivaltime = parseFloat(
                            $(
                                '[data-process="' +
                                    cpu_process +
                                    '"][class="arrival_time"]'
                        ...
                        $('#P' + cpu_process + '_TAT').append(
                            GLOBAL_endTime - curr_arrivaltime
                        );
                        $('#P' + cpu_process + '_WT').append(
                            GLOBAL_startTime - curr_arrivaltime
                        );
                ...
                if (cpu_process == null) {
                    if (ready_queue.length > 0) {
                        if (GLOBAL_bubbleStart != null) {
                            var bubble_width =
                                ((i - GLOBAL_bubbleStart) /
                                    GET_BURSTTIME_SUM()) *
                                80;
                            $('#gantt_chart').append(
                                '<div class="bubble" style="background-color: white; width: ' +
                                    bubble_width +
                                    '%;  color: black;">BUBBLE<br/>' +
                                    GLOBAL_bubbleStart +
                                    ' - ' +
                                    i +
                                    '</div>'
                        ...
                        cpu_bursttime = parseFloat(
                            ready_queue[0].split('?')[1]
                        );
                        ready_queue.shift();
                        GLOBAL_startTime = i;
...
function PROCESS_ARRIVALS(time) {
    ...
    $('.arrival_time').each(function (index) {
        ...
        if (curr_arrival_time == parseFloat(time)) {
            ...
            var curr_prio = parseFloat(
                $(
                    '[data-process="' + process_number + '"][class="priority"]'
                ).val()
            );
            ready_queue.push(
                process_number + '?' + curr_bursttime + '?' + curr_prio
            );
            console.log(
                time +
                    '\t: PR ARRVD : ' +
...
function checkValues() {
    ...
    $('.arrival_time').each(function (index) {
        if ($(this).val() == '' || !$.isNumeric($(this).val())) {
            $('#cust_console').append(
                'Please input a number for Arrival Time for Process P' +
                    (index + 1) +
                    '<br/>'
            );
            flag = false;
    ...
    $('.burst_time').each(function (index) {
        if ($(this).val() == '' || !$.isNumeric($(this).val())) {
            $('#cust_console').append(
                'Please input a number for Burst Time for Process P' +
                    (index + 1) +
                    '<br/>'
            );
            flag = false;
    ...
    $('.priority').each(function (index) {
        if ($(this).val() == '' || !$.isNumeric($(this).val())) {
            $('#cust_console').append(
                'Please input a number for Priority for Process P' +
                    (index + 1) +
                    '<br/>'
            );
            flag = false;
...
function SORT_READY_QUEUE() {
    ready_queue.sort(function (a, b) {
        // sort queue by lowest priority first
